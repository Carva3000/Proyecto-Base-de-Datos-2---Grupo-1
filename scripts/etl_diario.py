...
from config import DB_CONFIG, EXCEL_DIARIO, LOG_FILE
...
def get_max_date_in_dw(cursor):
    """Get the maximum date already loaded in the data warehouse"""
    cursor.execute("SELECT MAX(Fecha) FROM Dim_Tiempo")
    result = cursor.fetchone()
    return result[0] if result[0] else date(2000, 1, 1)
...
def load_daily_data(excel_path, sheet_name='Ventas_Dia', target_date=None):
    """Load only new daily data (incremental)"""
    logger.info(f"Iniciando carga diaria desde {excel_path}")
    ...
    logger.info(f"Leídas {len(rows)} filas del Excel diario")
    ...
    try:
        max_date_dw = get_max_date_in_dw(cursor)
        logger.info(f"Última fecha en DW: {max_date_dw}")
        
        if target_date <= max_date_dw:
            logger.info(f"La fecha {target_date} ya está cargada. Omitiendo carga.")
            return
        ...
        for row in rows:
            ...
            fact_transacciones[key_trans].add(data['nro_factura'])
        ...
        if processed == 0:
            logger.info(f"No hay datos nuevos para la fecha {target_date}")
            return
        
        logger.info(f"Procesados {processed} registros para {target_date}")
        ...
        execute_values(cursor, 
            """INSERT INTO Fact_Ventas_Prendas (Id_Prenda, Id_Tiempo, Cantidad_Vendida, Monto_Vendido)
               ...
               Cantidad_Vendida = Fact_Ventas_Prendas.Cantidad_Vendida + EXCLUDED.Cantidad_Vendida,
               Monto_Vendido = Fact_Ventas_Prendas.Monto_Vendido + EXCLUDED.Monto_Vendido""",
            valores_vp)
        ...
        execute_values(cursor,
            """INSERT INTO Fact_Ingresos_Temporales (Id_Tiempo, Monto_Total_Venta)
               ...
               Monto_Total_Venta = Fact_Ingresos_Temporales.Monto_Total_Venta + EXCLUDED.Monto_Total_Venta""",
            valores_ing)
        ...
        execute_values(cursor,
            """INSERT INTO Fact_Transacciones (Id_FormaPago, Id_Tiempo, Cantidad_Transacciones)
               ...
               Cantidad_Transacciones = Fact_Transacciones.Cantidad_Transacciones + EXCLUDED.Cantidad_Transacciones""",
            valores_trans)
        ...
        execute_values(cursor,
            """INSERT INTO Fact_Venta_Promociones (Id_Prenda, Id_Promocion, Id_Tiempo, Cantidad_Vendida, Monto_Vendido)
               ...
               Cantidad_Vendida = Fact_Venta_Promociones.Cantidad_Vendida + EXCLUDED.Cantidad_Vendida,
               Monto_Vendido = Fact_Venta_Promociones.Monto_Vendido + EXCLUDED.Monto_Vendido""",
            valores_promo)
        ...
        execute_values(cursor,
            """INSERT INTO Hechos_Rendimiento (Id_Vendedor, Id_Tiempo, Monto_Total_Venta)
               ...
               Monto_Total_Venta = Hechos_Rendimiento.Monto_Total_Venta + EXCLUDED.Monto_Total_Venta""",
            valores_rend)
        
        conn.commit()
        logger.info("Carga diaria completada exitosamente")
        
    except Exception as e:
        conn.rollback()
        logger.error(f"Error en carga diaria: {e}")
        raise
    finally:
        cursor.close()
        conn.close()
...
if __name__ == '__main__':
    import argparse
    parser = argparse.ArgumentParser(description='Carga diaria incremental')
    parser.add_argument('--date', type=str, help='Fecha a cargar (YYYY-MM-DD), por defecto ayer')
    ...
    load_daily_data(EXCEL_DIARIO, target_date=target_date)